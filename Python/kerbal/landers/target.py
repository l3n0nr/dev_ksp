...
text3 = panel.add_text("Launching now..")
...
## minimal altitude ascend, for begin modifications in landing target
limit_ascend = 15000

# while altitude() < 17500:
while altitude() < limit_ascend:
    """Initial ascent."""

    # set pitch angle to 85 degrees (engine gimbal only)
    # ap.target_pitch = 85.
    # ap.target_pitch = 90.                           ## PITCH 0
    ap.target_pitch = 70.
    ...
    ap.target_heading = ap.target_pitch + ((ap.target_pitch*2) / math.pi) * math.atan2(y, abs(x))
...
while altitude() > 15:
    ...
    ## altura minima para abrir trem de pouso e mirar no retrograde
    land_altitude = 1000
    ...
    if altitude() > land_altitude:
        ...
        # else:
        #     text3.content = "Waiting for suicide burn"
        #     vessel.control.sas = True
        #     vessel.control.gear = True
        #     vessel.control.sas_mode = conn.space_center.SASMode.retrograde
        #     vessel.control.throttle = 0
